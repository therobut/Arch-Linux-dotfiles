...
$A(_,$[0],$2(photoURL),$[1],s3('icon-grnotes.png'),$[2],$L(220),$[3],$L(221,screen_name),$[4]);
$A(_,templates.forms.buttonField({label:_L(212),id: "shareCancelAction", secondary: true } ));
$A(_,templates.forms.buttonField({label:_L(213),  id: "shareSendAction",   secondary: false } ));
$A(_,$[5],s3('loading.gif'),$[6],$L(222),$[7]);
return _.join($E);
}
...
$A(_,$[14],$L(223,'a href="'+devhd.str.toSafeAttr(anEntry.getAlternateLink())+'"', anEntry.getTitle(),
	                       (["a href=\"",_L(224),"\""]).join("")),$[7]);
return _.join($E);
}
$=[
