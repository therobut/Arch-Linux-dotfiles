...
$A(_,templates.page.base.h2Delimiter( _L(225),_L(226,stats.clicks) ));
$A(_,$[18],anEntry.getId(),$[19]);
return _.join($E);
}
$P.facebook=function(anEntry){var _=[];
$A(_,templates.page.base.h2Delimiter( _L(227) ));
$A(_,$[20],anEntry.getAlternateLink(),$[21]);
return _.join($E);
}
$P.rss2comments=function(anEntry, rss2){var _=[];
 var items=rss2.channel.item || []
$A(_,templates.page.base.h2Delimiter( _L(228) ));
if(items.length == 0){
    $A(_,$[22],$L(229),$[23]);
}else{
    $A(_,$[24],$L(230,items.length),$[25]);
    for(var i=items.length - 1; i >= 0; i--){
        $A(_,$[26],$L(231,('a href="'+items[i].link+'" target="_comment"'), items[i].creator),$[27]);
        var comment = items[i].content || items[i].description
        $A(_,$[28],$3(comment),$[29],templates.page.base.dateAsDuration ( items[i].pubDate ),$[30]);
    }
    $A(_,$[31],$2(anEntry.getAlternateLink()),$[32],$L(232),$[12]);
}
return _.join($E);
}
$P.atom2comments=function(anEntry, feed){var _=[];
 var items=feed.entry || []
$A(_,$[33],templates.page.base.h2Delimiter( _L(228) ),$[34]);
if(items.length == 0){
    $A(_,$[22],$L(229),$[23]);
}else{
    $A(_,$[35],$L(233,items.length),$[36]);
    for(var i = items.length - 1; i >= 0; i--){
        $A(_,$[37],$L(231,'span class="commenterName"', items[i].author),$[38],$1(items[i].author),$[39],$3(items[i].content || items[i].description),$[29],templates.page.base.dateAsDuration( items[i].published ),$[30]);
    }
    $A(_,$[31],$2(anEntry.getAlternateLink()),$[32],$L(232),$[12]);
}
return _.join($E);
...
$A(_,templates.page.base.h2Delimiter( _L(228) ));
$A(_,$[40],$L(234),$[41],$2(anEntry.getAlternateLink()),$[42],$L(235),$[12]);
return _.join($E);
}
$P.commentsError=function(code, msg){var _=[];
$A(_,templates.page.base.h2Delimiter( _L(228) ));
$A(_,$[40],$L(234),$[43],code,$[44],msg,$[45]);
return _.join($E);
}
