...
$A(_,$[0],s3('loading.gif'),$[1],$L(216),$[2]);
return _.join($E);
}
$P.form=function(message, screen_name, photoURL){var _=[];
$A(_,$[3],$2(photoURL),$[4],$1(message),$[5],s3('icon-ning.png'),$[6],$L(217),$[7],$L(218,'span id="title_hint" class="fieldHint" style="font-weight: normal; padding-left: 3px"',
			   screen_name),$[8]);
$A(_,templates.forms.buttonField ( { label: _L(212), id: "openSocialCancelAction", secondary: true } ));
$A(_,templates.forms.buttonField ( { label: _L(213),  id: "openSocialSendAction",   secondary: false } ));
$A(_,$[9],s3('loading.gif'),$[10],$L(219),$[2]);
return _.join($E);
}
$=[
