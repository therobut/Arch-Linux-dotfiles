...
( function ()
{
		...
		this.reader.askEntry( 			this.entryId,
										function( anEntry )
										{
											try
											{
												...
												{
													that.analytics.trackEvent( "promoted", "open", anEntry.getFeedId() );
												}
												else
												{
	...
	// College Humor
	// http://www.collegehumor.com/video/6914356/jake-and-amir-audition
	var RE_CH_VIDEO = /video\/([0-9]+)/i;
	...
	function doPostProcessing()
	{
		...
		if( bodyElem != null && ( bodyElem.clientHeight > this.home.getDocument().body.clientHeight - 280 || this.entry.listContentVisuals().length > 1 || ( this.entry.listContentVisuals().length > 0 && this.entry.asText() > 300 ) ) )
		{
			var bWikiElem = this.element( this.entry.getId() + "_bottomWikiWidget");
			if( bWikiElem != null )
				bWikiElem.style.display = "block";
		}
	...
	EntryControl.processSelection = function (event)
	{
		try
		{
			...
			this.preferences.askPreference( "contextualMenu", function( contextualMenu ){
				...
				LOG.trace(510,"range present, will process selection");
